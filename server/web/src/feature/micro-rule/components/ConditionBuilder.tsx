...
import type { LogicalOperator, TargetType, MatchType } from "@/types/rule"
...
const OperatorBadge = ({ operator, onClick, className = "" }) => {
    const badgeStyles =
        operator === "AND"
...
const ActionButtons = ({ addSimpleCondition, addCompositeCondition, t }) => (
...
export function ConditionBuilder({
    ...
    const conditionType = form.watch(`${path}.type`) as "simple" | "composite"

    // If composite condition, get operator and child conditions
    const operator = form.watch(`${path}.operator`) as LogicalOperator
    const conditions = form.watch(`${path}.conditions`) || []

    // If simple condition, get target and match type
    const target = form.watch(`${path}.target`) as TargetType
    const matchType = form.watch(`${path}.match_type`) as MatchType
    ...
    const addSimpleCondition = () => {
        ...
        const currentConditions = form.getValues(`${path}.conditions`) || []
        form.setValue(`${path}.conditions`, [
            ...currentConditions,
            {
                type: "simple",
                target: "source_ip",
    ...
    const toggleOperator = () => {
        if (conditionType !== "composite") return
        const newOperator = operator === "AND" ? "OR" : "AND"
        form.setValue(`${path}.operator`, newOperator)
    }
    ...
    const removeCondition = (index: number) => {
        ...
        form.setValue(`${path}.conditions`, currentConditions)
    }
